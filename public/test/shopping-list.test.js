describe("ShoppingList", function () {
  ...
  describe(".render", function () {
    ...
    it("should concat each, item using their render method, in <ul> tag", function () {
      let cart = new ShoppingList();
      let food = new ShoppingListItem("apples", "food");
      let book = new ShoppingListItem("comics", "marvel");
      let drink = new ShoppingListItem("coke", "beverage");
      ...
      cart.addItem(book);
      cart.addItem(drink);
      expect(cart.render()).to.equal(`<ul><li class="completed_${food.isDone}"><span>${food.name}</span><span>${food.description}</span></li><li class="completed_${book.isDone}"><span>${book.name}</span><span>${book.description}</span></li><li class="completed_${drink.isDone}"><span>${drink.name}</span><span>${drink.description}</span></li></ul>`);
    });
  });
});
